...
import {
  ...
  type TimezoneMode,
  formatTimestamp,
} from "./components/TimezoneSelector";
    ...
    <div className="app-container">
      <div className="sidebar">
        ...
        <div className="stats-grid grid grid-cols-2 gap-4 mb-8">
          ...
          <div
            className={`stat-card ${connected ? "text-green-500" : "text-red-500"}`}
          >
            <span className="text-dim text-xs block">LINK STATUS</span>
            <span className="text-xs font-bold uppercase">
              {connected ? "SECURE" : "INTERRUPTED"}
            </span>
          </div>
        </div>

        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
          <h2 className="text-xs font-bold text-dim mb-4 tracking-widest uppercase">
            Terrain Topology
          </h2>
            ...
            {nodes.map((node) => (
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span
                    className={`badge ${node.is_healthy ? "badge-cyan" : "badge-red"}`}
                  >
                ...
                <div className="flex gap-3 text-[10px] text-dim mb-1">
                  <div className="flex items-center gap-1">
                    <Zap size={10} /> {node.uplink}
                  </div>
                  <div className="flex items-center gap-1">
                    <Activity size={10} /> {node.latency_ms.toFixed(0)}ms
                  </div>
                </div>
      ...
      <main className="map-viewport">
        ...
        >
          <div
            className={`w-2 h-2 rounded-full ${connected ? "bg-green-500 shadow-[0_0_10px_#39ff14]" : "bg-red-500"}`}
          />
          <span className="tracking-widest uppercase">
            {connected
              ? "Secured QUIC Uplink Active"
              : "Attempting Cipher Handshake..."}
          </span>
          <div className="ml-4 border-l border-glass pl-4 flex gap-4 text-xs">
            <span className="flex items-center gap-1">
              <Globe size={12} /> Global Mesh: {stats.healthy}/{nodes.length}
            </span>
          </div>
        </motion.div>
      </main>
